...
<a href="/watch/{{ $animeSlug }}/{{ $episode['number'] }}" class="group flex items-center gap-3 p-2.5 bg-[#151515] hover:bg-[#1A1A1A] border border-zinc-800/80 hover:border-violet-500/50 rounded-xl transition-all duration-200">
    ...
    <div class="relative w-28 md:w-32 aspect-video rounded-lg overflow-hidden bg-zinc-900 flex-shrink-0">
        ...
        <div class="absolute inset-0 bg-black/40 group-hover:bg-violet-600/30 transition-colors flex items-center justify-center">
            <div class="w-8 h-8 rounded-full bg-violet-600 text-white flex items-center justify-center shadow-md transform group-hover:scale-110 transition-transform">
        ...
        @if(!empty($episode['watched']))
            <div class="absolute top-1.5 left-1.5 px-1.5 py-0.5 bg-violet-600 text-white text-[9px] font-bold rounded flex items-center gap-1 shadow-sm">
    ...
    <div class="flex-grow min-w-0">
        <div class="flex items-center justify-between gap-2">
            <span class="text-xs font-bold text-violet-400">EP {{ sprintf('%02d', $episode['number']) }}</span>
        ...
        <div class="flex items-center gap-2 mt-1.5 text-[10px] text-zinc-400">
            ...
            <span class="px-1.5 py-0.5 bg-zinc-900 border border-zinc-800 rounded font-semibold text-violet-300">HD 1080p</span>
        </div>
    </div>
