...
<nav class="md:hidden fixed bottom-0 left-0 right-0 z-40 h-16 bg-[#0B0B0B]/90 backdrop-blur-xl border-t border-zinc-800/80 px-2 flex items-center justify-around">
    ...
    <!-- Profile Action Trigger (Opens Profile Modal) -->
    <button type="button" 
            id="mobile-profile-trigger" 
            data-nav-target="profile" 
            class="mobile-nav-item-link flex flex-col items-center gap-1 text-[11px] font-medium transition-colors {{ request()->is('profile') ? 'text-[#E63946] font-semibold active' : 'text-zinc-400 hover:text-zinc-200' }}">
        @auth
            <div class="relative">
                <img src="{{ auth()->user()->avatarUrl() }}" 
                     alt="{{ auth()->user()->name }}" 
                     class="w-5 h-5 rounded-full object-cover border border-[#E63946]"
                     onerror="this.onerror=null;this.src='https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name) }}&background=E63946&color=fff';">
                <span class="absolute -bottom-0.5 -right-0.5 w-2 h-2 rounded-full bg-emerald-500 border border-[#0B0B0B]"></span>
            </div>
            <span>Profil</span>
        @else
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
