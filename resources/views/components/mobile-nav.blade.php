...
<nav class="md:hidden fixed bottom-0 left-0 right-0 z-40 h-16 bg-[#0B0B0B]/90 backdrop-blur-xl border-t border-zinc-800/80 px-2 flex items-center justify-around">
    <!-- Home -->
    <a href="/" class="flex flex-col items-center gap-1 text-[11px] font-medium transition-colors {{ request()->is('/') ? 'text-violet-400 font-semibold' : 'text-zinc-400 hover:text-zinc-200' }}">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Explore -->
    <a href="/anime" class="flex flex-col items-center gap-1 text-[11px] font-medium transition-colors {{ request()->is('anime*') ? 'text-violet-400 font-semibold' : 'text-zinc-400 hover:text-zinc-200' }}">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Watchlist -->
    <a href="/watchlist" class="flex flex-col items-center gap-1 text-[11px] font-medium transition-colors {{ request()->is('watchlist') ? 'text-violet-400 font-semibold' : 'text-zinc-400 hover:text-zinc-200' }}">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Profile/Login -->
    <a href="/login" class="flex flex-col items-center gap-1 text-[11px] font-medium transition-colors {{ request()->is('login') || request()->is('register') ? 'text-violet-400 font-semibold' : 'text-zinc-400 hover:text-zinc-200' }}">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
