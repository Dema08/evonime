    ...
    <div class="max-w-[1400px] mx-auto space-y-8 text-[#F5F0E6]">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[#1A1A1A] border-2 border-[#F5F0E6] shadow-[6px_6px_0px_#F5F0E6] p-6">
            <div class="space-y-1">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-xs font-mono text-zinc-400">STATUS: SECURE // ONLINE</span>
                </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-[#1A1A1A] border-2 border-[#F5F0E6] shadow-[4px_4px_0px_#F5F0E6] p-5 space-y-2 relative overflow-hidden">
                ...
                <div class="text-4xl font-black text-[#F5F0E6]" style="font-family: 'Anton', sans-serif;">{{ $totalAnime }} SERIES</div>
                <p class="text-[11px] text-zinc-400 font-bold">+2 added this week</p>
            </div>
            ...
            <div class="bg-[#1A1A1A] border-2 border-[#F5F0E6] shadow-[4px_4px_0px_#F5F0E6] p-5 space-y-2 relative overflow-hidden">
                ...
                <div class="text-4xl font-black text-[#F5F0E6]" style="font-family: 'Anton', sans-serif;">{{ $totalEpisodes }} EPS</div>
            ...
            <div class="bg-[#1A1A1A] border-2 border-[#F5F0E6] shadow-[4px_4px_0px_#F5F0E6] p-5 space-y-2 relative overflow-hidden">
                ...
                <span class="text-xs font-black uppercase tracking-wider text-zinc-400 bg-[#141414] px-2 py-0.5 border border-[#F5F0E6]">GENRES & THEMES</span>
                <div class="text-4xl font-black text-[#F5F0E6]" style="font-family: 'Anton', sans-serif;">{{ $totalGenres }} GENRES</div>
                <p class="text-[11px] text-zinc-400 font-bold">Fully categorized</p>
            </div>
        ...
        <div class="bg-[#1A1A1A] border-2 border-[#F5F0E6] shadow-[6px_6px_0px_#F5F0E6] p-6 space-y-6">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b-2 border-[#F5F0E6] pb-4">
                ...
                <a href="/anime" class="manga-button-primary px-5 py-2.5 text-xs font-black rounded-lg text-white text-center">
                    + ADD NEW ANIME
                </a>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-[#141414] border-2 border-[#F5F0E6] text-xs font-black uppercase tracking-wider text-[#F5F0E6]">
                            ...
                            <th class="p-3 border-r-2 border-[#F5F0E6]">Episodes</th>
                            <th class="p-3">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y-2 divide-zinc-800 text-xs font-bold text-zinc-300">
                        @foreach(array_slice($animeList, 0, 8) as $anime)
                            <tr class="hover:bg-zinc-900/60 transition-colors">
                                <td class="p-3 border-r-2 border-zinc-800">
                                    <img src="{{ $anime['poster'] }}" alt="{{ $anime['title'] }}" class="w-10 h-14 object-cover border border-[#F5F0E6]">
                                </td>
                                <td class="p-3 border-r-2 border-zinc-800 text-white font-black">
                                    {{ $anime['title'] }}
                                </td>
                                <td class="p-3 border-r-2 border-zinc-800">
                                    <span class="px-2 py-0.5 bg-[#141414] border border-[#F5F0E6] text-[10px]">{{ $anime['type'] }}</span>
                                </td>
                                ...
                                <td class="p-3 space-x-2">
                                    <a href="/anime/{{ $anime['slug'] }}" class="px-3 py-1 bg-[#141414] hover:bg-[#E63946] text-[#F5F0E6] hover:text-white border border-[#F5F0E6] font-black transition-colors inline-block">VIEW</a>
                                    <button type="button" onclick="window.showToast('Edit mode not enabled in demo')" class="px-3 py-1 bg-[#141414] hover:bg-zinc-800 text-zinc-400 hover:text-white border border-zinc-700 font-bold transition-colors">EDIT</button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
